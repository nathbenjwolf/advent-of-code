...
char input[] = "oaoe rxeq vssdqtu xrk cjv yaoqp loo,\
...
int isAnagram( char* word1, char* word2 ) {
    ...
    for( int i=0; word1[i] != '\0'; i++ ) {
        alphaInst1[ word1[i] - 'a' ]++;
    }

    for( int i=0; word2[i] != '\0'; i++ ) {
        alphaInst2[ word2[i] - 'a' ]++;
    }

    for( int i=0; i<26; i++ ) {
        if( alphaInst1[i] != alphaInst2[i] ) {
            return 0;
        }
    }
...
int isValidPassphrase( char * passphrase ) {
    char * delim = " ";
    char * endStr;
    char * wordsInPass[ 50 ];

    int i = 0;
    wordsInPass[ i ] = strtok_r( passphrase, delim, &endStr );

    while( wordsInPass[ i ] != NULL ) {
        for( int j = 0; j < i; j++ ) {
            if( isAnagram( wordsInPass[j], wordsInPass[i] ) != 0 ) {
                return 0;
            }
        }
        wordsInPass[ ++i ] = strtok_r( NULL, delim, &endStr );
    }
...
int main( void ) {
    ...
    passphrase = strtok_r( input, delim, &endStr );
    ...
    while( passphrase != NULL ) {
        ...
        passphrase = strtok_r( NULL, delim, &endStr );
    }

    printf( "Total valid passphrase: %d\n", validPass );
