...
struct node * createNewNode( char * regName ) {
    struct node * myNode = malloc( sizeof(struct node) );
    myNode->myReg.name = regName;
    myNode->myReg.value = 0;
    myNode->nextNode = NULL;
    return myNode;
}

struct node *findReg( char * regName ) {
    struct node * curNode = head;
    struct node * tailNode;
    while( curNode != NULL ) {
        if( strcmp( curNode->myReg.name, regName ) == 0 ) {
            return curNode;
        }
        tailNode = curNode;
    ...
    // Register not created yet
    struct node *newNode = createNewNode( regName );
    tailNode->nextNode = newNode;

    return newNode;
}

struct reg *getReg( char * regName ) {
    struct node *myNode;
    if( head == NULL ) {
        myNode = createNewNode( regName );
        head = myNode;
    } else {
        myNode = findReg( regName );
    }

    return &myNode->myReg;
}

void executeCmd( struct reg * modReg, int isInc, int modNum,
                 const struct reg * condReg, char *inequality, int condNum ) {

    // Check conditional
    int condTrue = 0;
    if( strcmp( inequality, ">=" ) == 0 ) {
        if( condReg->value >= condNum ) condTrue = 1;
        ...
        printf( "Encountered unsupported inequality: %s\n", inequality );
    }
...
void parseCmd( char * input ) {
    char * endStr;
    char * delim = " ";

    // register name
    char * modRegName = strtok_r( input, delim, &endStr );
    struct reg *modReg = getReg( modRegName );

    // + or -
    char * incStr = strtok_r( NULL, delim, &endStr );
    int isInc = strcmp( incStr, "dec" );

    // amount
    int modNum = atoi( strtok_r( NULL, delim, &endStr ) );

    // if (ignored)
    strtok_r( NULL, delim, &endStr );

    // conditional register
    char * condRegName = strtok_r( NULL, delim, &endStr );
    struct reg *condReg = getReg( condRegName );

    // inequality
    char * inequality = strtok_r( NULL, delim, &endStr );

    // compared value
    int condNum = atoi( strtok_r( NULL, delim, &endStr ) );

    executeCmd( modReg, isInc, modNum, condReg, inequality, condNum );
}

struct reg findMaxReg() {
    struct node * curNode = head;
    struct reg maxReg = curNode->myReg;
    while( curNode != NULL ) {
        if( curNode->myReg.value > maxReg.value ) {
            maxReg = curNode->myReg;
        }
...
void printRegs() {
    struct node * curNode = head;
    while( curNode != NULL ) {
        printf( "Register: %s, value: %d\n", curNode->myReg.name, curNode->myReg.value );
...
int main( void ) {
    ...
    char * cmd = strtok_r( input, delim, &endStr );

    while( cmd != NULL ) {
        parseCmd( cmd );
        struct reg maxReg = findMaxReg();
        if( maxReg.value > maxVal ) {
            maxVal = maxReg.value;
        }
        cmd = strtok_r( NULL, delim, &endStr );
    }

    printf( "Max value ever held was: %d\n", maxVal );
