...
//char inputStr[] = "5 9 2 8,9 4 7 3,3 8 6 5";

//char inputStr[] = "123 456 789, 432 123 456";

int lineDifference( char * input ) {
    char delim[] = " ";
    char * currentNum;
    char * endStr;
    int min;
    ...
    currentNum = strtok_r( input, delim, &endStr );
    min = max = atoi( currentNum );

    while( 1 ) {
        currentNum = strtok_r( NULL, delim, &endStr );
        if( currentNum == NULL ) {
            break;
        }
        int num = atoi( currentNum );
...
int lineDivision( char * input ) {
    ...
    int i = 0;
    lineNums[i++] = atoi( strtok_r( input, delim, &endStr ) );
    ...
    while( 1 ) {
        ...
        lineNums[i] = atoi( currentNum );

        for( int j=0; j<i; j++ ) {
            if( lineNums[j] % lineNums[i] == 0 ) {
                return lineNums[j] / lineNums[i];
            }
            if( lineNums[i] % lineNums[j] == 0 ) {
                return lineNums[i] / lineNums[j];
            }
        }
...
int main( int argc, char **argv ) {
    ...
    totalSum += lineDivision( strtok_r( inputStr, delim, &endStr ) );

    while( 1 ) {
        char *token = strtok_r( NULL, delim, &endStr );
        printf( "token: %s\n", token );
        if( token == NULL ) {
            break;
        }
        totalSum += lineDivision( token );
    }
